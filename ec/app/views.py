...
class checkout(View):
    def get(self,request):
        ...
        #{'id': 'order_KoVaJizv4WnbRq', 'entity': 'order', 'amount': 41500, 'amount_paid': 0, 'amount_due': 41500, 'currency': 'INR', 'receipt': 'order_rcptid_12', 'offer_id': None, 'status': 'created', 'attempts': 0, 'notes': [], 'created_at': 1670304369}
...
def remove_cart(request):
    if request.method == 'GET':
        ...
        # Store the total quantity of all matching items
        total_quantity = sum(item.quantity for item in cart_items)
        
        # Delete all matching cart items
        cart_items.delete()

        # Calculate the updated cart value
        user = request.user
        cart = Cart.objects.filter(user=user)
        amount = sum(p.quantity * p.product.discounted_price for p in cart)
        totalamount = amount + 40  # Assuming 40 is some kind of constant fee
